...
$(document).ready(function() {
    ...
    table = $('#table-1').DataTable({
        ...
        "ajax": {
            ...
            "type": "POST",
            "data": function ( data ) {
                data.tglawal = tglawal($('#tgl').val());
                data.tglakhir = tglakhir($('#tgl').val());
            }
        },
...
function winOnLoad() {
    var beban = $("#beban").val();
    if (beban == 'Non Project') {
        nprojek();
    } else if (beban == 'Project') {
        projek();
    }
}
...
function tglawal(tglA){
    var res = tglA.split("-");
    var format = res[0];
    return format;
}

function tglakhir(tglAK){
    var res = tglAK.split("-");
    var format = res[1];
    return format;
}
...
function nprojek() {
    $("#btnNprojek").removeClass();
    $("#btnNprojek").addClass("btn btn-primary");

    $("#btnProjek").removeClass();
    $("#btnProjek").addClass("btn btn-secondary");

    $("#beban").val('Non Project');
    $("#projekFrom").fadeOut();
    $("#checkNP").show();
    $("#checkP").hide();

    $("#idP").val('').trigger("chosen:updated");
}

function projek() {
    $("#btnProjek").removeClass();
    $("#btnProjek").addClass("btn btn-success");

    $("#btnNprojek").removeClass();
    $("#btnNprojek").addClass("btn btn-secondary");
...
function validateForm() {
    var kb = document.forms["myForm"]["kb"].value;
    var tgl = document.forms["myForm"]["tgl"].value;
    var realisasi = document.forms["myForm"]["realisasi"].value;
    var ket = document.forms["myForm"]["ket"].value;

    if (kb == '') {
        validasi('Pilih Kategori Biaya!', 'warning');
        return false;
    } else if (tgl == '') {
        validasi('Tanggal wajib di isi!', 'warning');
        return false;
    } else if (realisasi == '') {
        validasi('Nilai Realisasi wajib di isi!', 'warning');
        return false;
    }
